...
| [`dsh-editor`](packages/dsh-editor/README.md) | **Editor tab** — text/code files open **editable** (Markdown included, with a **Preview ⇄ Edit** toggle to the rendered page), blank documents are created and named from the "+" guide, and the CodeMirror palette follows the app's light/dark theme. The rendered page resets what the preview's plain-text scrollport imposes, so Markdown draws as a document. | alpha `0.1.0-alpha.8` |
| [`dsh-gittree`](packages/dsh-gittree/README.md) | **GitTree tab** — the workspace's git tree as a **read-only** page tab: every tracked file plus the changed/untracked ones with status badges, a path filter and a changed-only switch, and a **History** view whose commits open to their changed files. A row click opens the file through the ordinary address, so the editor or a shipped preview claims it. | alpha `0.1.0-alpha.1` |
| [`dsh-themes`](packages/dsh-themes/README.md) | **Themes** — the header's Light / Dark / System button (the same switch as Settings → Appearance) plus the pack's appearance overrides: the always-light **Markdown paper** the rendered view is drawn on, and the **Markdown chrome** rule that hides the preview header's viewer menu on Markdown tabs. | alpha `0.1.0-alpha.3` |

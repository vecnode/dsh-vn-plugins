...
| [`dsh-gittree`](packages/dsh-gittree/README.md) | **GitTree tab** — the workspace’s **commit history** as a **read-only** page tab: the branch and the current commit in its bar, the commit list below (id, subject, author, date), and a picked commit’s message and changed files — each opening through the ordinary address, so the editor or a shipped preview claims it. | alpha `0.1.0-alpha.2` |
